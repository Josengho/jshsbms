					...
					<td>
					  <c:choose>
					   ...
					   <c:when test="${item.deliveryState == 'delivering'}">
					   		<input type="button" onClick="fn_cancel_order('${item.orderId}')" class="btn btn-outline-primary btn-sm" value="배송중" disabled />
					   </c:when>
					   <c:when test="${item.deliveryState == 'finishedDelivering'}">
					   		<input type="button" onClick="fn_cancel_order('${item.orderId}')" class="btn btn-outline-primary btn-sm" value="배송완료" disabled />
					   </c:when>
					   <c:otherwise>
