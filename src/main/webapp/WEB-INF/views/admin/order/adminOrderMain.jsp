...
<script>
	...
	function fn_modify_order_state(orderId,deliveryState){
		
		
		var s_deliveryState = document.getElementById(deliveryState);
	          	...
	          	<tr>
					...
					<td width=10%>
						<select name="deliveryState" id="deliveryState">
							<option value="deliveryPrepared" <c:if test="${item.deliveryState=='deliveryPrepared' }"> </c:if>>배송준비중</option>
							<option value="delivering" <c:if test="${item.deliveryState=='delivering' }"> </c:if>>배송중</option>
							<option value="finishedDelivering" <c:if test="${item.deliveryState=='finishedDelivering' }"> </c:if>>배송완료</option>
							<option value="cancelOrder" <c:if test="${item.deliveryState=='cancelOrder' }"> </c:if>>주문취소</option>
							<option value="returningGoods" <c:if test="${item.deliveryState=='returningGoods' }"> </c:if>>반품</option>
						</select>
						<input type="button" value="배송수정" class="btn btn-outline-blue btn-xs" onClick="fn_modify_order_state('${item.orderId}','deliveryState')" />
					</td>				
					</tr>
				</c:forEach>
			  </c:otherwise>
